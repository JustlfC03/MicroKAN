...
@jit(cache=True)
def diff_divscale_exp_smt(dpdataset,mask,opt):
    fpref = glob.glob(os.path.join(os.path.join(dpdataset,opt.subj),opt.subpath,"*_spherical_mean.nii.gz"))

    if fpref is None:
        fpDwi = glob.glob(os.path.join(os.path.join(dpdataset,opt.subj),opt.subpath,"*_dwi.nii.gz"))[0]
        fpBval = glob.glob(os.path.join(os.path.join(dpdataset,opt.subj),opt.subpath,"*.bval"))[0]
        fpdelta = glob.glob(os.path.join(os.path.join(dpdataset,opt.subj),opt.subpath,"*.pulseWidth"))[0]
        fpDelta = glob.glob(os.path.join(os.path.join(dpdataset,opt.subj),opt.subpath,"*.diffusionTime"))[0]

        dwi = nb.load(fpDwi).get_fdata()
        mask = mask[:,:,:,None]
        bval = pd.read_csv(fpBval, header=None,sep=' ').values.squeeze()
        delta = np.loadtxt(fpdelta)
        Delta = np.loadtxt(fpDelta)
        scheme = torch.tensor([])
        
        # b0
        idx = np.where(bval<25)[0]
        b0 = np.zeros(dwi[:,:,:,0:1].shape)
        b0count_map = np.zeros(dwi[:,:,:,0:1].shape)
        signal_min_threshold = 2e-16
        for ii in range(0,len(idx)):
            diff_tmp = dwi[:,:,:,idx[ii]:idx[ii]+1] 
            b0 = b0 + diff_tmp
            b0count_map = b0count_map + (diff_tmp>signal_min_threshold)
        ...
        # nb.save(nb.Nifti1Image(b0, nb.load(fpDwi).affine), os.path.join('/autofs/cluster/connectome2/zjl/DIMOND/AxCaliberSMT_Data/sub-010','b0.nii.gz'))

        entire_mask = (b0count_map>0) * mask

        data_gt = b0

        for delta_val in np.unique(delta):
            for Delta_val in np.unique(Delta):
                for bval_val in np.delete(np.unique(bval),0):
                    idx = np.where((delta == delta_val) & (Delta == Delta_val) & (bval == bval_val))[0]
                    if idx.shape[0] == 0:
                        continue
                    tmp = torch.tensor([delta_val,Delta_val,bval_val]).unsqueeze(0)
                    scheme = torch.cat((scheme,tmp),dim=0)
                    data_tmp = np.zeros(dwi[:,:,:,0:1].shape)
                    data_tmp_count_map = np.zeros(dwi[:,:,:,0:1].shape)
                    for ii in range(len(idx)):
                        data_tmp = data_tmp + dwi[:,:,:,idx[ii]:idx[ii]+1]
                        data_tmp_count_map = data_tmp_count_map + (dwi[:,:,:,idx[ii]:idx[ii]+1]>signal_min_threshold)
                    entire_mask_tmp = (data_tmp_count_map>0) * mask
                    data_tmp_count_map[data_tmp_count_map==0]=1
                    data = data_tmp/data_tmp_count_map
                    entire_mask = np.concatenate([entire_mask,entire_mask_tmp],axis=-1)

                    data_gt = np.concatenate([data_gt,data],axis=3)

        # 写入spherical mean后的数据
        name = os.path.basename(fpDwi).split('.')[0]
        ref_name = name + '_spherical_mean.nii.gz'
        fpref = os.path.join(os.path.join(dpdataset,opt.subj),opt.subpath,ref_name)
        nb.save(nb.Nifti1Image(data_gt, nb.load(fpDwi).affine), fpref)
        in_features = data_gt.shape[-1]

    else:
        fpref = fpref[0]
        data_gt = nb.load(fpref).get_fdata()
        b0 = data_gt[:,:,:,0:1]

        entire_mask = np.repeat((b0>0) * mask[:,:,:,None], repeats=2, axis=3)
        scheme_path = os.path.join(os.path.join(dpdataset,opt.subj),opt.subpath,"scheme.txt")
        scheme = torch.tensor(np.loadtxt(scheme_path))
        in_features = data_gt.shape[-1]

    # normalize
    scale_num = np.percentile(b0[mask>0], 50)
    diffnorm = data_gt/scale_num
    b0scale = b0/scale_num

    return diffnorm,b0scale,entire_mask,scale_num,scheme,fpref,in_features
...
class NiftiFile3D_block_NODDI(Dataset):
    def __init__(self, dpSub, grayscale=True,use_tmask = False,use_wm_mask = False, useT1=False,useT2=False,aug=0,opt=None,SMT_feature = None):
        super().__init__()
        start = time.time()
        
        if opt.diffusionmodel == 'AxCaliberSMT':
            fpImg = glob.glob(os.path.join(dpSub,opt.subpath,"*.nii.gz"))[0]
        ...
        if use_tmask:
            ...
            fpMask = glob.glob(os.path.join(dpSub,opt.subpath, "*_wm_mask.nii.gz"))[0]
        else:
            print("brainmask")
        ...
        ## normalize
        if "SMT" in opt.diffusionmodel:
            img_norm = SMT_feature['diffnorm']
            b0scale = SMT_feature['b0scale']
            entire_mask = SMT_feature['entire_mask']
            b0scalenum = SMT_feature['scale_num']
        else:
        ...
        if "SMT" not in opt.diffusionmodel:
            ...
            self.supplementary = {'bvec':torch.tensor([])}
...
class FastImageNormalDataset3D(torch.utils.data.Dataset):
    def __init__(self, dataset, patch_size=(16, 16), sidelength=None, random_coords=False,
        ...
        if opt.dataset in ['NODDI','DWI3DManyFast10','budadata','MGHAxCaliber']:
...
class PartANODDIFT(Dataset):
    def __init__(self, dpSubs, subj,grayscale=True,use_tmask = False, use_wm_mask = False, fploadlist = None,useT1 = False,useT2 = False,aug=0,opt=None,SMT_feature = None):
        super().__init__()
        self.img_datasets = []
        self.idxdict = {}
        numdataset = 0
        ...
        print('dpSub: ', dpSub)
        img_dataset = FastImageNormalDataset3D(NiftiFile3D_block_NODDI(dpSub,grayscale=grayscale,use_tmask = use_tmask, use_wm_mask = use_wm_mask, aug=aug,opt=opt, SMT_feature = SMT_feature),opt=opt)
