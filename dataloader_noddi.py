...
class PartANODDIFT(Dataset):
    ...
    def load_data(self, subj, base_path):
        ...
        fpfiso = os.path.join(fpsub, "diff-Gt-matlab", f"{subj}noddiparameters_fiso.nii")
        fpficvf = os.path.join(fpsub, "diff-Gt-matlab", f"{subj}noddiparameters_ficvf.nii")
        fpodi = os.path.join(fpsub, "diff-Gt-matlab", f"{subj}noddiparameters_odi.nii")
        ...
        fiso = nb.load(fpfiso).get_fdata()
        ficvf = nb.load(fpficvf).get_fdata()
        odi = nb.load(fpodi).get_fdata()
        GT = np.stack([fiso,ficvf,odi], axis=3)
