...
def EvalDtiNew(fpCal,fpRef,tmask,subj,metrics=["FA","MD","L1","RD"]):
    error = {}
    for ii in range(0,len(metrics)):
        ref = nb.load(os.path.join(fpRef,subj,"dti",subj +"_dti_"+ metrics[ii] +".nii.gz")).get_fdata()
        cal = nb.load(os.path.join(fpCal,subj +"_"+ metrics[ii] +".nii.gz")).get_fdata()
        # print("ref.shape",ref.shape)
        # print("cal.shape",cal.shape)
        if metrics[ii] in ["V1","V2","V3"]:
            # print("ref.shape",ref.shape)            
...
if __name__ == "__main__":
    for subj in ['mwu100307', 'mwu114419', 'mwu101107', 'mwu113619', 'mwu103818', 'mwu105115', 'mwu106016', 'mwu110411', 'mwu115320', 'mwu111716']:
        mask = nb.load(f"/Data/shared_data/HCP_MWU/{subj}/aparc+aseg-diff/MASK_BRAIN_TISSUE.nii.gz").get_fdata()
        result_dict = EvalDtiNew("/Data/Users/cyf/DWI/DeepDTI-main/result","/Data/shared_data/HCP_MWU/",mask,subj)
